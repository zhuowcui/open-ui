...
static void draw_section_label(OuiSkCanvas canvas,
                               OuiSkFont font,
                               OuiSkPaint paint,
                               float x,
                               float y,
...
static void draw_basic_shapes(OuiSkCanvas canvas,
                              OuiSkFont label_font,
                              OuiSkPaint label_paint,
                              float ox,
                              float oy) {
  draw_section_label(canvas, label_font, label_paint, ox, oy - 10, "1. Basic Shapes");
...
static void draw_paths(OuiSkCanvas canvas,
                       OuiSkFont label_font,
                       OuiSkPaint label_paint,
                       float ox,
                       float oy) {
  draw_section_label(canvas, label_font, label_paint, ox, oy - 10, "2. Path Operations");
  ...
  for (int i = 0; i < 5; i++) {
    ...
    if (i == 0)
      oui_sk_path_move_to(star, x, y);
    else
      oui_sk_path_line_to(star, x, y);
  }
  ...
  oui_sk_path_cubic_to(bezier, ox + 120, oy - 20, ox + 200, oy + 80, ox + 220, oy);
  oui_sk_paint_set_style(p, OUI_SK_PAINT_STYLE_STROKE);
  oui_sk_paint_set_stroke_width(p, 3.0f);
  ...
  /* SVG path */
  OuiSkPath svg =
      oui_sk_path_from_svg_string("M 260 10 Q 280 0 300 10 Q 320 20 300 30 Q 280 40 260 30 Z");
  if (svg) {
    oui_sk_paint_set_style(p, OUI_SK_PAINT_STYLE_FILL);
...
static void draw_paint_styles(OuiSkCanvas canvas,
                              OuiSkFont label_font,
                              OuiSkPaint label_paint,
                              float ox,
                              float oy) {
  ...
  OuiSkStrokeCap caps[] = {OUI_SK_STROKE_CAP_BUTT, OUI_SK_STROKE_CAP_ROUND,
                           OUI_SK_STROKE_CAP_SQUARE};
  for (int i = 0; i < 3; i++) {
    oui_sk_paint_set_stroke_cap(p, caps[i]);
    oui_sk_canvas_draw_line(canvas, ox + 470 + i * 40, oy + 5, ox + 470 + i * 40, oy + 45, p);
  }
...
static void draw_gradients(OuiSkCanvas canvas,
                           OuiSkFont label_font,
                           OuiSkPaint label_paint,
                           float ox,
                           float oy) {
  ...
  OuiSkShader lin = oui_sk_shader_linear_gradient(s, e, colors3, pos3, 3, OUI_SK_TILE_MODE_CLAMP);
  ...
  OuiSkShader rad =
      oui_sk_shader_radial_gradient(center, 40, colors3, pos3, 3, OUI_SK_TILE_MODE_CLAMP);
  oui_sk_paint_set_shader(p, rad);
  oui_sk_canvas_draw_circle(canvas, ox + 240, oy + 30, 40, p);
  oui_sk_shader_destroy(rad);

  /* Sweep */
  OuiSkColor colors4[] = {rgb(66, 133, 244), rgb(15, 157, 88), rgb(244, 180, 0), rgb(219, 68, 55)};
  float pos4[] = {0.0f, 0.33f, 0.66f, 1.0f};
  OuiSkPoint sc = {ox + 370, oy + 30};
  OuiSkShader sw = oui_sk_shader_sweep_gradient(sc, colors4, pos4, 4);
...
static void draw_text(OuiSkCanvas canvas,
                      OuiSkFont label_font,
                      OuiSkPaint label_paint,
                      float ox,
                      float oy) {
  draw_section_label(canvas, label_font, label_paint, ox, oy - 10, "5. Text Rendering");

  OuiSkTypeface tf = oui_sk_typeface_create_from_name("sans-serif", OUI_SK_FONT_STYLE_NORMAL);
  OuiSkPaint p = oui_sk_paint_create();
  oui_sk_paint_set_anti_alias(p, true);
  ...
  for (int i = 0; i < 4; i++) {
    ...
    oui_sk_canvas_draw_text(canvas, buf, strlen(buf), ox, y + sizes[i], f, p);
    y += sizes[i] + 8;
    oui_sk_font_destroy(f);
  }

  /* Bold text */
  OuiSkTypeface tf_bold = oui_sk_typeface_create_from_name("sans-serif", OUI_SK_FONT_STYLE_BOLD);
  if (tf_bold) {
    OuiSkFont fb = oui_sk_font_create(tf_bold, 20.0f);
    if (fb) {
      oui_sk_paint_set_color(p, rgb(0, 100, 200));
      const char* bold_text = "Bold Text";
      oui_sk_canvas_draw_text(canvas, bold_text, strlen(bold_text), ox + 200, oy + 24, fb, p);
  ...
  /* Italic text */
  OuiSkTypeface tf_italic =
      oui_sk_typeface_create_from_name("sans-serif", OUI_SK_FONT_STYLE_ITALIC);
  if (tf_italic) {
    OuiSkFont fi = oui_sk_font_create(tf_italic, 20.0f);
    if (fi) {
      oui_sk_paint_set_color(p, rgb(200, 50, 50));
      const char* italic_text = "Italic Text";
      oui_sk_canvas_draw_text(canvas, italic_text, strlen(italic_text), ox + 200, oy + 52, fi, p);
...
static void draw_filters(OuiSkCanvas canvas,
                         OuiSkFont label_font,
                         OuiSkPaint label_paint,
                         float ox,
                         float oy) {
  ...
  /* Blur filter */
  OuiSkImageFilter blur = oui_sk_image_filter_blur(4.0f, 4.0f, OUI_SK_TILE_MODE_CLAMP);
  oui_sk_paint_set_color(p, rgb(66, 133, 244));
  ...
  /* Drop shadow */
  OuiSkImageFilter shadow =
      oui_sk_image_filter_drop_shadow(4.0f, 4.0f, 3.0f, 3.0f, rgba(0, 0, 0, 128));
  oui_sk_paint_set_color(p, rgb(15, 157, 88));
  ...
  OuiSkMaskFilter mask = oui_sk_mask_filter_blur(OUI_SK_BLUR_STYLE_NORMAL, 3.0f);
  oui_sk_paint_set_color(p, rgb(244, 180, 0));
...
static void draw_transforms(OuiSkCanvas canvas,
                            OuiSkFont label_font,
                            OuiSkPaint label_paint,
                            float ox,
                            float oy) {
...
static void draw_clipping(OuiSkCanvas canvas,
                          OuiSkFont label_font,
                          OuiSkPaint label_paint,
                          float ox,
                          float oy) {
  draw_section_label(canvas, label_font, label_paint, ox, oy - 10, "8. Clipping (Rect / Path)");
  ...
  OuiSkShader gsh = oui_sk_shader_linear_gradient(gs, ge, gc, gp, 2, OUI_SK_TILE_MODE_CLAMP);
...
static void draw_blending(OuiSkCanvas canvas,
                          OuiSkFont label_font,
                          OuiSkPaint label_paint,
                          float ox,
                          float oy) {
  draw_section_label(canvas, label_font, label_paint, ox, oy - 10, "9. Alpha Blending");
  ...
  for (int i = 0; i < 3; i++) {
    ...
    oui_sk_paint_set_color(p, rgb(33, 33, 33));
    if (small) {
      oui_sk_canvas_draw_text(canvas, names[i], strlen(names[i]), bx + 5, oy + 60, small, p);
    }
  }

  if (small)
    oui_sk_font_destroy(small);
  if (tf)
    oui_sk_typeface_destroy(tf);
  oui_sk_paint_destroy(p);
}
...
static void draw_image_ops(OuiSkCanvas canvas,
                           OuiSkFont label_font,
                           OuiSkPaint label_paint,
                           float ox,
                           float oy) {
...
int main(int argc, char* argv[]) {
  ...
  /* Title font */
  OuiSkTypeface tf = oui_sk_typeface_create_from_name("sans-serif", OUI_SK_FONT_STYLE_BOLD);
  OuiSkFont title_font = oui_sk_font_create(tf, 28.0f);

  /* Section label font */
  OuiSkTypeface tf2 = oui_sk_typeface_create_from_name("sans-serif", OUI_SK_FONT_STYLE_BOLD);
  ...
  if (title_font) {
    oui_sk_canvas_draw_text(canvas, title, strlen(title), 30, 40, title_font, label_paint);
  }
  ...
  void* data = NULL;
  size_t size = 0;
  OuiSkStatus status = oui_sk_image_encode(snapshot, OUI_SK_IMAGE_FORMAT_PNG, 100, &data, &size);
  ...
  if (title_font)
    oui_sk_font_destroy(title_font);
  if (label_font)
    oui_sk_font_destroy(label_font);
  oui_sk_paint_destroy(label_paint);
  if (tf)
    oui_sk_typeface_destroy(tf);
  if (tf2)
    oui_sk_typeface_destroy(tf2);
