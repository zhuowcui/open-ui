...
static void draw_scene(OuiSkCanvas canvas) {
  ...
  OuiSkShader header_grad = oui_sk_shader_linear_gradient(
      (OuiSkPoint){0, 0}, (OuiSkPoint){600, 0}, header_colors, NULL, 2, OUI_SK_TILE_MODE_CLAMP);
  ...
  oui_sk_canvas_draw_text(canvas, "Hello, Open UI!", 15, 24, 52, font_title, paint);

  /* ── Card with shadow ───────────────────────────────────── */
  OuiSkColor shadow_color = oui_sk_color_make(0, 0, 0, 60);
  OuiSkImageFilter card_shadow = oui_sk_image_filter_drop_shadow(0, 4, 8, 8, shadow_color);
  oui_sk_paint_set_color(paint, oui_sk_color_make(255, 255, 255, 255));
  oui_sk_paint_set_image_filter(paint, card_shadow);
  OuiSkRRect card = oui_sk_rrect_make(oui_sk_rect_make_xywh(30, 110, 540, 180), 12, 12);
  ...
  OuiSkRRect green_rr = oui_sk_rrect_make(oui_sk_rect_make_xywh(155, 145, 100, 70), 8, 8);
  ...
  OuiSkTypeface tf_normal =
      oui_sk_typeface_create_from_name("sans-serif", OUI_SK_FONT_STYLE_NORMAL);
  ...
  oui_sk_canvas_draw_text(canvas, desc, strlen(desc), 60, 262, font_body, paint);

  /* ── Rainbow gradient bar ───────────────────────────────── */
  OuiSkColor rainbow[7] = {
      oui_sk_color_make(255, 0, 0, 255),   oui_sk_color_make(255, 127, 0, 255),
      oui_sk_color_make(255, 255, 0, 255), oui_sk_color_make(0, 255, 0, 255),
      oui_sk_color_make(0, 0, 255, 255),   oui_sk_color_make(75, 0, 130, 255),
      oui_sk_color_make(148, 0, 211, 255),
  };
  OuiSkShader rainbow_grad = oui_sk_shader_linear_gradient(
      (OuiSkPoint){30, 0}, (OuiSkPoint){570, 0}, rainbow, NULL, 7, OUI_SK_TILE_MODE_CLAMP);
  oui_sk_paint_set_shader(paint, rainbow_grad);
  OuiSkRRect bar = oui_sk_rrect_make(oui_sk_rect_make_xywh(30, 320, 540, 20), 10, 10);
  ...
  oui_sk_canvas_draw_text(canvas, footer, strlen(footer), 30, 370, font_small, paint);
  ...
  if (tf_normal)
    oui_sk_typeface_destroy(tf_normal);
  if (tf_bold)
...
int main(int argc, char** argv) {
  ...
  void* png_data = NULL;
  size_t png_size = 0;
  OuiSkStatus status = oui_sk_image_encode(image, OUI_SK_IMAGE_FORMAT_PNG, 0, &png_data, &png_size);
