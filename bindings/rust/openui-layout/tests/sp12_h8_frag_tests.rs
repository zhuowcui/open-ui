...
#[test] fn mc_fill_balance() { assert_eq!(balance_columns(&[lu(100), lu(100), lu(100)], &vec![false; 3], 3, lu(1000)), lu(100)); }
...
#[test] fn mc_balance_uneven() { assert_eq!(balance_columns(&[lu(50), lu(80), lu(60), lu(40)], &vec![false; 4], 2, lu(1000)), lu(115)); }
#[test] fn mc_balance_convergence() { assert_eq!(balance_columns(&[lu(100); 6], &vec![false; 6], 3, lu(1000)), lu(200)); }
...
#[test] fn mc_balance_single_col() { assert_eq!(balance_columns(&[lu(100), lu(200)], &vec![false; 2], 1, lu(1000)), lu(300)); }
#[test] fn mc_balance_empty() { assert_eq!(balance_columns(&[], &vec![false; 1], 3, lu(1000)), lu(0)); }
#[test] fn mc_balance_large_child() { assert_eq!(balance_columns(&[lu(500), lu(50), lu(50)], &vec![false; 3], 2, lu(1000)), lu(300)); }
#[test] fn mc_balance_all_same() { assert_eq!(balance_columns(&[lu(100); 6], &vec![false; 6], 3, lu(1000)), lu(200)); }
#[test] fn mc_balance_2c3i() { assert_eq!(balance_columns(&[lu(100); 3], &vec![false; 3], 2, lu(1000)), lu(150)); }
...
#[test] fn mc_bal_7i3c() { let h = balance_columns(&[lu(10), lu(20), lu(30), lu(40), lu(50), lu(60), lu(70)], &vec![false; 7], 3, lu(1000)); assert!(h.raw() > 0 && h <= lu(1000)); }
#[test] fn mc_bal_constrained() { let h = balance_columns(&[lu(100); 3], &vec![false; 3], 2, lu(120)); assert!(h.raw() > 0); }
...
#[test] fn mc_bal_identical() { assert_eq!(balance_columns(&[lu(50); 10], &vec![false; 10], 5, lu(1000)), lu(100)); }
#[test] fn mc_bal_one_large() { let mut c = vec![lu(10); 9]; c.push(lu(500)); assert_eq!(balance_columns(&c, &vec![false; c.len()], 2, lu(1000)), lu(295)); }
...
#[test] fn nest_balance_within() { assert_eq!(balance_columns(&[lu(30); 6], &vec![false; 6], 2, lu(500)), lu(90)); }
...
#[test] fn edge_bal_single_large() { assert_eq!(balance_columns(&[lu(1000)], &vec![false; 1], 3, lu(2000)), LayoutUnit::from_raw(21334)); }
#[test] fn edge_bal_zero_h() { assert_eq!(balance_columns(&[lu(0); 3], &vec![false; 3], 2, lu(500)), lu(0)); }
...
#[test] fn edge_bal_max_small() { assert_eq!(balance_columns(&[lu(100), lu(200), lu(50)], &vec![false; 3], 3, lu(50)), LayoutUnit::from_raw(7467)); }
...
#[test] fn edge_bal_10i4c() { let h = balance_columns(&[lu(25); 10], &vec![false; 10], 4, lu(1000)); assert!(h >= lu(50) && h <= lu(100)); }
...
#[test] fn extra_mc_balance_4c8i() {
    assert_eq!(balance_columns(&[lu(50); 8], &vec![false; 8], 4, lu(1000)), lu(100));
}
#[test] fn extra_mc_balance_5c10i() {
    assert_eq!(balance_columns(&[lu(20); 10], &vec![false; 10], 5, lu(1000)), lu(40));
}
#[test] fn extra_mc_balance_2c_desc() {
    // 200, 100, 50 → total 350. 2 cols. With fragmentation: 175.
    let h = balance_columns(&[lu(200), lu(100), lu(50)], &vec![false; 3], 2, lu(1000));
    assert_eq!(h, lu(175));
}
...
#[test] fn extra_mc_balance_single_item_2c() {
    assert_eq!(balance_columns(&[lu(200)], &vec![false; 1], 2, lu(1000)), lu(100));
}
...
#[test] fn extra_edge_balance_2c_single_item() {
    assert_eq!(balance_columns(&[lu(300)], &vec![false; 1], 2, lu(500)), lu(150));
}
#[test] fn extra_edge_bal_max_capped() {
    // With a small max_height, the binary search still converges.
    let h = balance_columns(&[lu(100); 4], &vec![false; 4], 2, lu(100));
...
#[test] fn extra_mc_balance_all_ones() {
    assert_eq!(balance_columns(&[lu(1); 100], &vec![false; 100], 10, lu(1000)), lu(10));
}
...
#[test] fn extra_mc_bal_3c_asymmetric() {
    ...
    let h = balance_columns(&[lu(300), lu(100), lu(100)], &vec![false; 3], 3, lu(1000));
...
#[test] fn extra_mc_bal_2c2i_equal() {
    assert_eq!(balance_columns(&[lu(150), lu(150)], &vec![false; 2], 2, lu(1000)), lu(150));
}
