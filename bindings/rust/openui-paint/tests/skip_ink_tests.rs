...
use openui_paint::decoration_painter::{self, DecorationPhase};
use openui_style::*;
...
fn paint_and_check(text: &str, style: &ComputedStyle, phase: DecorationPhase) -> Surface {
...
fn skip_ink_none_underline_draws_continuous() {
    ...
    assert!(
        ...
        "Underline with skip-ink:none should be visible"
    );
}
...
fn skip_ink_none_overline_draws_continuous() {
    ...
    assert!(
        has_non_white_pixels(&mut surface),
        "Overline with skip-ink:none should be visible"
    );
}
...
fn skip_ink_auto_underline_creates_visible_decoration() {
    ...
    assert!(
        has_non_white_pixels(&mut surface),
        "Auto skip-ink should still produce visible underline"
    );
}
...
fn skip_ink_auto_overline_creates_visible_decoration() {
    ...
    assert!(
        has_non_white_pixels(&mut surface),
        "Auto skip-ink overline should be visible"
    );
}
...
fn skip_ink_all_underline_visible() {
    ...
    assert!(
        has_non_white_pixels(&mut surface),
        "All skip-ink should produce visible underline"
    );
}
...
fn skip_ink_all_overline_visible() {
    ...
    assert!(
        has_non_white_pixels(&mut surface),
        "All skip-ink overline should be visible"
    );
}
...
fn skip_ink_not_applied_to_line_through_all() {
    ...
    assert!(
        has_non_white_pixels(&mut surface),
        "Line-through should be visible with skip-ink:all"
    );
}
...
fn dilation_capped_at_13px() {
    ...
    assert_eq!(
        thin_dilation, 1.6,
        "Thin line dilation should equal thickness"
    );

    let thick_dilation = 20.0_f32.min(13.0);
    assert_eq!(
        thick_dilation, 13.0,
        "Thick line dilation should cap at 13px"
    );
...
fn single_space_character() {
    ...
    if sr.width() > 0.0 {
        assert!(
            has_non_white_pixels(&mut surface),
            "Space should still draw underline"
        );
    }
}
...
fn long_text_with_descenders() {
    ...
    assert!(
        has_non_white_pixels(&mut surface),
        "Descender text should still show underline segments"
    );
}
...
fn skip_ink_auto_underline_and_overline() {
    ...
    assert!(
        has_non_white_pixels(&mut surface),
        "Both underline and overline should be visible"
    );
}
