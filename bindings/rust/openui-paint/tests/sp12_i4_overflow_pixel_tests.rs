...
fn overflow_hidden_clips_child_horizontally() {
    ...
    // Inside parent: content visible
    assert!(
        has_non_white_in_region(&mut s, 10, 10, 80, 180),
        "Content inside parent should be visible"
    );
    // Right of parent: clipped
    assert!(
        region_is_white(&mut s, 110, 0, 100, 200),
...
fn overflow_hidden_clips_both_axes() {
    ...
    // Inside: visible
    assert!(
        has_non_white_in_region(&mut s, 10, 10, 130, 80),
        "Content inside parent should render"
    );
    // Right of parent: clipped
    assert!(
        region_is_white(&mut s, 160, 0, 100, 100),
        "Content beyond right edge should be clipped"
    );
    // Below parent: clipped
    assert!(
        region_is_white(&mut s, 0, 110, 150, 100),
...
fn overflow_hidden_clips_vertical_small_parent() {
    ...
    assert!(
        has_non_white_in_region(&mut s, 5, 5, 40, 20),
        "Small parent should show content within bounds"
    );
    assert!(
        region_is_white(&mut s, 0, 40, 50, 50),
...
fn overflow_hidden_clips_horizontal_narrow_parent() {
    ...
    assert!(
        has_non_white_in_region(&mut s, 5, 5, 50, 90),
        "Content within 60px parent should be visible"
    );
    assert!(
        region_is_white(&mut s, 70, 0, 100, 100),
...
fn overflow_hidden_clips_large_parent() {
    ...
    assert!(
        has_non_white_in_region(&mut s, 50, 50, 300, 200),
        "Large parent should show content"
    );
    assert!(
        region_is_white(&mut s, 0, 310, 400, 50),
...
fn overflow_hidden_child_bg_clipped_at_bottom_edge() {
    ...
    assert!(
        pixel_is_not_white(&mut s, 100, 40),
        "Center of parent should have content"
    );
    assert!(
        pixel_is_white(&mut s, 100, 90),
...
fn overflow_hidden_parent_bg_renders_fully() {
    ...
    // Beyond parent: white
    assert!(
        region_is_white(&mut s, 210, 0, 100, 100),
...
fn overflow_hidden_with_padding_clips_inside_padding_box() {
    ...
    assert!(
        has_non_white_in_region(&mut s, 15, 15, 190, 90),
        "Content within padded parent should be visible"
    );
    // Beyond the border box: white
    assert!(
        region_is_white(&mut s, 230, 0, 100, 120),
...
fn overflow_hidden_multiple_children_clipped() {
    ...
    assert!(
        region_is_white(&mut s, 0, 70, 200, 30),
...
fn overflow_hidden_zero_height_clips_everything() {
    ...
    assert!(
        region_is_white(&mut s, 0, 5, 200, 50),
...
fn overflow_hidden_zero_width_clips_everything() {
    ...
    assert!(
        region_is_white(&mut s, 5, 0, 100, 100),
...
fn overflow_hidden_with_border_clips_inside_border() {
    ...
    assert!(
        region_is_white(&mut s, 220, 0, 100, 110),
...
fn overflow_hidden_clips_bottom_right_corner_only() {
    ...
    assert!(
        region_is_white(&mut s, 0, 210, 200, 50),
        "Content below parent should be clipped"
    );
}
...
fn overflow_visible_enum_not_scrollable() {
    assert!(
        !Overflow::Visible.is_scrollable(),
...
fn overflow_visible_child_renders_within_parent() {
    ...
    assert!(
        has_non_white_in_region(&mut s, 10, 10, 140, 70),
...
fn overflow_visible_child_extends_horizontally() {
    ...
    assert!(
        has_non_white_in_region(&mut s, 110, 10, 100, 180),
...
fn overflow_visible_multiple_children() {
    ...
    assert!(
        has_non_white_in_region(&mut s, 10, 55, 180, 40),
        "Second child should be visible"
    );
    assert!(
        has_non_white_in_region(&mut s, 10, 105, 180, 40),
...
fn overflow_visible_content_not_restricted_by_parent_size() {
    ...
    assert!(
        has_non_white_in_region(&mut s, 10, 60, 180, 50),
...
fn overflow_visible_both_axes_overflow() {
    ...
    assert!(
        has_non_white_in_region(&mut s, 10, 10, 80, 60),
        "Content inside parent should be visible"
    );
    assert!(
        has_non_white_in_region(&mut s, 110, 10, 50, 60),
        "Content extending right should be visible"
    );
    assert!(
        has_non_white_in_region(&mut s, 10, 90, 80, 50),
...
fn overflow_all_clipping_modes_clip_identically() {
    ...
    for mode in &modes {
        ...
        assert!(
            region_is_white(&mut s, 0, 110, 200, 50),
            "overflow:{:?} should clip content below parent",
            mode
        );
    }
}
...
fn max_height_with_overflow_hidden_clips_content() {
    ...
    assert!(
        region_is_white(&mut s, 0, 90, 200, 50),
...
fn nested_overflow_hidden_inner_clips_further() {
    ...
    assert!(
        region_is_white(&mut s, 310, 0, 100, 200),
...
fn three_level_nesting_overflow_hidden() {
    ...
    assert!(
        region_is_white(&mut s, 310, 0, 100, 250),
...
fn overflow_hidden_with_colored_borders_visible() {
    ...
    // Content inside should be visible
    assert!(
        has_non_white_in_region(&mut s, 10, 10, 190, 80),
...
fn overflow_scroll_with_nested_boxes() {
    ...
    assert!(
        has_non_white_in_region(&mut s, 10, 60, 180, 30),
...
fn multiple_overflow_containers_stacked() {
    ...
    assert!(
        has_non_white_in_region(&mut s, 10, 90, 180, 60),
    ...
    assert!(
        region_is_white(&mut s, 0, 170, 200, 50),
...
fn all_overflow_modes_render_something() {
    ...
    for mode in &modes {
        ...
        assert!(
            has_visible_content(&mut s),
            "overflow:{:?} should produce visible output with colored boxes",
            mode
        );
    }
}
...
fn overflow_hidden_child_partially_visible() {
    ...
    assert!(
        has_non_white_in_region(&mut s, 10, 75, 180, 20),
...
fn overflow_hidden_clips_at_varying_widths() {
    ...
    for width in [50.0_f32, 100.0, 200.0, 300.0] {
        ...
        assert!(
            pixel_is_white(&mut s, check_x, 40),
            "Content at x={} should be clipped for parent width={}",
            check_x,
            width
        );
    }
...
fn overflow_hidden_clips_at_varying_heights() {
    for height in [40.0_f32, 80.0, 120.0, 200.0] {
        let (_, mut s) =
            build_overflow_test(Overflow::Hidden, 200.0, height, 200.0, height + 200.0);
        let check_y = (height as i32) + 20;
        assert!(
            pixel_is_white(&mut s, 100, check_y),
            "Content at y={} should be clipped for parent height={}",
            check_y,
            height
        );
    }
...
fn overflow_hidden_with_min_width_child() {
    ...
    assert!(
        region_is_white(&mut s, 110, 0, 100, 100),
