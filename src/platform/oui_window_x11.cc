...
static void recreate_surface(OuiWindow_t* win) {
  ...
  auto backend_rt = GrBackendRenderTargets::MakeGL(
      win->width, win->height, sample_count > 1 ? sample_count : 0, stencil_bits, fbo_info);

  SkColorType color_type = kRGBA_8888_SkColorType;
  win->surface = SkSurfaces::WrapBackendRenderTarget(
      win->gr_context.get(), backend_rt, kBottomLeft_GrSurfaceOrigin, color_type, nullptr, nullptr);
...
OuiWindow oui_window_create(const char* title, int width, int height, OuiSkBackend backend) {
  if (width <= 0 || height <= 0)
    return nullptr;
  if (!title)
    title = "Open UI";
  ...
  int num_configs = 0;
  GLXFBConfig* fb_configs = glXChooseFBConfig(display, screen, visual_attribs, &num_configs);
  ...
  // Create GLX context
  GLXContext glx_ctx = glXCreateNewContext(display, fb_config, GLX_RGBA_TYPE, nullptr, True);
  if (!glx_ctx) {
  ...
  if (!win->canvas_valid) {
    fprintf(stderr,
            "oui_window: Warning — GPU surface not created, "
...
bool oui_window_poll_event(OuiWindow window, OuiEvent* event) {
  ...
  while (XPending(window->display)) {
    ...
    switch (xev.type) {
      ...
      case KeyPress:
      case KeyRelease: {
        event->type = (xev.type == KeyPress) ? OUI_EVENT_KEY_DOWN : OUI_EVENT_KEY_UP;
        ...
        if (xev.type == KeyPress) {
          ...
          if (window->xic) {
            ...
            len = Xutf8LookupString(window->xic, &xev.xkey, buf, sizeof(buf) - 1, &keysym, &status);
            // Handle XBufferOverflow: retry with a larger heap buffer
            if (status == XBufferOverflow && len > 0) {
              char* heap_buf = static_cast<char*>(malloc(static_cast<size_t>(len) + 1));
              if (heap_buf) {
                memset(heap_buf, 0, static_cast<size_t>(len) + 1);
                len = Xutf8LookupString(window->xic, &xev.xkey, heap_buf, len, &keysym, &status);
                text_buf = heap_buf;
              }
            }
          } else {
            // Fallback to XLookupString (locale-encoded, no IME)
            len = XLookupString(&xev.xkey, buf, sizeof(buf) - 1, &keysym, nullptr);
          }
          ...
          if (len > 0 && static_cast<unsigned char>(text_buf[0]) >= 32) {
            ...
            if (copy_len < len) {
              ...
              int i = copy_len - 1;
              while (i >= 0 && (static_cast<unsigned char>(text_buf[i]) & 0xC0) == 0x80) {
                i--;
              }
            ...
            if (copy_len > 0) {
              ...
              memcpy(window->pending_text_event.text_input.text, text_buf, copy_len);
            }
          }
