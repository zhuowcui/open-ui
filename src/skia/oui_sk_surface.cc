...
OuiSkSurface oui_sk_surface_create_gpu(OuiSkGpuContext gpu_ctx, int width, int height) {
  if (!gpu_ctx || width <= 0 || height <= 0)
    return nullptr;

#ifdef SK_GANESH
  SkImageInfo info = SkImageInfo::MakeN32Premul(width, height);
  sk_sp<SkSurface> surface =
      SkSurfaces::RenderTarget(gpu_ctx->context.get(), skgpu::Budgeted::kNo, info);
  if (!surface)
    return nullptr;
...
OuiSkStatus oui_sk_surface_read_pixels(OuiSkSurface surface,
                                       void* dst,
