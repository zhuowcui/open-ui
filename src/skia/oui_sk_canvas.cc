...
void oui_sk_canvas_concat_matrix(OuiSkCanvas canvas, const OuiSkMatrix* matrix) {
  if (!canvas || !canvas->canvas || !matrix)
    return;
...
void oui_sk_canvas_draw_rrect(OuiSkCanvas canvas, const OuiSkRRect* rrect, OuiSkPaint paint) {
  if (!canvas || !canvas->canvas || !rrect || !paint)
    return;
  canvas->canvas->drawRRect(to_sk_rrect(*rrect), paint->paint);
}

void oui_sk_canvas_draw_circle(OuiSkCanvas canvas,
                               float cx,
                               float cy,
                               float radius,
                               OuiSkPaint paint) {
...
void oui_sk_canvas_draw_line(OuiSkCanvas canvas,
                             float x0,
                             float y0,
                             float x1,
                             float y1,
...
void oui_sk_canvas_draw_image(OuiSkCanvas canvas,
                              OuiSkImage image,
                              float x,
                              float y,
                              OuiSkPaint paint) {
  if (!canvas || !canvas->canvas || !image)
    return;
  const SkPaint* p = paint ? &paint->paint : nullptr;
  canvas->canvas->drawImage(image->image, x, y, SkSamplingOptions(), p);
}

void oui_sk_canvas_draw_image_rect(OuiSkCanvas canvas,
                                   OuiSkImage image,
                                   const OuiSkRect* src,
                                   const OuiSkRect* dst,
                                   OuiSkPaint paint) {
  if (!canvas || !canvas->canvas || !image || !dst)
    return;
  ...
  if (src) {
    ...
    canvas->canvas->drawImageRect(image->image, sk_src, sk_dst, SkSamplingOptions(), p,
                                  SkCanvas::kStrict_SrcRectConstraint);
  } else {
    canvas->canvas->drawImageRect(image->image, sk_dst, SkSamplingOptions(), p);
  }
}

void oui_sk_canvas_draw_text(OuiSkCanvas canvas,
                             const char* text,
                             size_t len,
                             float x,
                             float y,
                             OuiSkFont font,
                             OuiSkPaint paint) {
  if (!canvas || !canvas->canvas || !text || !font || !paint)
    return;
...
void oui_sk_canvas_draw_text_blob(OuiSkCanvas canvas,
                                  OuiSkTextBlob blob,
                                  float x,
                                  float y,
                                  OuiSkPaint paint) {
  if (!canvas || !canvas->canvas || !blob || !paint)
    return;
