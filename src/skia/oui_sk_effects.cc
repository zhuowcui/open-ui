...
OuiSkShader oui_sk_shader_linear_gradient(OuiSkPoint start,
                                          OuiSkPoint end,
...
OuiSkShader oui_sk_shader_radial_gradient(OuiSkPoint center,
                                          float radius,
                                          const OuiSkColor* colors,
                                          const float* positions,
                                          int count,
...
OuiSkShader oui_sk_shader_sweep_gradient(OuiSkPoint center,
                                         const OuiSkColor* colors,
                                         const float* positions,
                                         int count) {
  if (!colors || count < 2)
...
OuiSkShader oui_sk_shader_image(OuiSkImage image, OuiSkTileMode tile_x, OuiSkTileMode tile_y) {
  if (!image || !image->image)
    return nullptr;

  sk_sp<SkShader> shader = image->image->makeShader(to_sk_tile_mode(tile_x),
                                                    to_sk_tile_mode(tile_y), SkSamplingOptions());
  if (!shader)
    return nullptr;
...
OuiSkImageFilter oui_sk_image_filter_blur(float sigma_x, float sigma_y, OuiSkTileMode tile_mode) {
  sk_sp<SkImageFilter> filter =
      SkImageFilters::Blur(sigma_x, sigma_y, to_sk_tile_mode(tile_mode), nullptr);
  if (!filter)
    return nullptr;
...
OuiSkImageFilter oui_sk_image_filter_drop_shadow(float dx,
                                                 float dy,
                                                 float sigma_x,
                                                 float sigma_y,
                                                 OuiSkColor color) {
  sk_sp<SkImageFilter> filter =
      SkImageFilters::DropShadow(dx, dy, sigma_x, sigma_y, to_sk_color(color), nullptr);
  if (!filter)
    return nullptr;
...
OuiSkImageFilter oui_sk_image_filter_color_filter(OuiSkColorFilter color_filter) {
  if (!color_filter || !color_filter->filter)
    return nullptr;

  sk_sp<SkImageFilter> filter = SkImageFilters::ColorFilter(color_filter->filter, nullptr);
  if (!filter)
    return nullptr;
...
OuiSkImageFilter oui_sk_image_filter_compose(OuiSkImageFilter outer, OuiSkImageFilter inner) {
  if (!outer || !inner)
    return nullptr;

  sk_sp<SkImageFilter> filter = SkImageFilters::Compose(outer->filter, inner->filter);
  if (!filter)
    return nullptr;
...
OuiSkMaskFilter oui_sk_mask_filter_blur(OuiSkBlurStyle style, float sigma) {
  sk_sp<SkMaskFilter> filter = SkMaskFilter::MakeBlur(static_cast<SkBlurStyle>(style), sigma);
  if (!filter)
    return nullptr;
...
OuiSkColorFilter oui_sk_color_filter_blend(OuiSkColor color, OuiSkBlendMode mode) {
  sk_sp<SkColorFilter> filter = SkColorFilters::Blend(
      to_sk_color4f({static_cast<float>(color.r) / 255.0f, static_cast<float>(color.g) / 255.0f,
                     static_cast<float>(color.b) / 255.0f, static_cast<float>(color.a) / 255.0f}),
      nullptr, to_sk_blend_mode(mode));
  if (!filter)
    return nullptr;
...
OuiSkColorFilter oui_sk_color_filter_compose(OuiSkColorFilter outer, OuiSkColorFilter inner) {
  if (!outer || !inner)
    return nullptr;

  sk_sp<SkColorFilter> filter = SkColorFilters::Compose(outer->filter, inner->filter);
  if (!filter)
    return nullptr;
