...
OuiSkTypeface oui_sk_typeface_create_from_name(const char* family_name,
                                               OuiSkFontStylePreset style) {
  sk_sp<SkFontMgr> mgr = oui_sk_get_font_manager();
  sk_sp<SkTypeface> tf = mgr->matchFamilyStyle(family_name, to_sk_font_style(style));
  if (!tf)
    return nullptr;
...
OuiSkTypeface oui_sk_typeface_create_from_data(const void* data, size_t size, int index) {
  if (!data || size == 0)
    return nullptr;
...
OuiSkTextBlob oui_sk_text_blob_create(const char* text, size_t len, OuiSkFont font) {
  if (!text || !font)
    return nullptr;

  sk_sp<SkTextBlob> blob = SkTextBlob::MakeFromText(text, len, font->font, SkTextEncoding::kUTF8);
  if (!blob)
    return nullptr;
...
OuiSkTextBlob oui_sk_text_shape(const char* text,
                                size_t len,
                                OuiSkFont font,
                                float width,
  ...
  try {
    ...
    TextAlign sk_align;
    switch (align) {
      case OUI_SK_TEXT_ALIGN_LEFT:
        sk_align = TextAlign::kLeft;
        break;
      case OUI_SK_TEXT_ALIGN_CENTER:
        sk_align = TextAlign::kCenter;
        break;
      case OUI_SK_TEXT_ALIGN_RIGHT:
        sk_align = TextAlign::kRight;
        break;
      default:
        sk_align = TextAlign::kLeft;
        break;
    }
    TextDirection sk_dir =
        (dir == OUI_SK_TEXT_DIRECTION_RTL) ? TextDirection::kRtl : TextDirection::kLtr;
    ...
    if (!unicode) {
      ...
      sk_sp<SkTextBlob> blob =
          SkTextBlob::MakeFromText(text, len, font->font, SkTextEncoding::kUTF8);
      if (!blob)
        return nullptr;
      auto* b = new (std::nothrow) OuiSkTextBlob_t();
      if (!b)
