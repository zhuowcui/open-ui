...
OUI_SK_API OuiSkSurface oui_sk_surface_create_gpu(OuiSkGpuContext gpu_ctx, int width, int height);
...
OUI_SK_API OuiSkStatus oui_sk_surface_read_pixels(OuiSkSurface surface,
                                                  void* dst,
...
OUI_SK_API void oui_sk_canvas_concat_matrix(OuiSkCanvas canvas, const OuiSkMatrix* matrix);

OUI_SK_API void oui_sk_canvas_clip_rect(OuiSkCanvas canvas, const OuiSkRect* rect, bool anti_alias);
OUI_SK_API void oui_sk_canvas_clip_rrect(OuiSkCanvas canvas,
                                         const OuiSkRRect* rrect,
                                         bool anti_alias);
OUI_SK_API void oui_sk_canvas_clip_path(OuiSkCanvas canvas, OuiSkPath path, bool anti_alias);

OUI_SK_API void oui_sk_canvas_clear(OuiSkCanvas canvas, OuiSkColor color);

OUI_SK_API void oui_sk_canvas_draw_rect(OuiSkCanvas canvas,
                                        const OuiSkRect* rect,
                                        OuiSkPaint paint);
OUI_SK_API void oui_sk_canvas_draw_rrect(OuiSkCanvas canvas,
                                         const OuiSkRRect* rrect,
                                         OuiSkPaint paint);
OUI_SK_API void oui_sk_canvas_draw_circle(OuiSkCanvas canvas,
                                          float cx,
                                          float cy,
                                          float radius,
                                          OuiSkPaint paint);
OUI_SK_API void oui_sk_canvas_draw_oval(OuiSkCanvas canvas,
                                        const OuiSkRect* rect,
                                        OuiSkPaint paint);
OUI_SK_API void oui_sk_canvas_draw_path(OuiSkCanvas canvas, OuiSkPath path, OuiSkPaint paint);
OUI_SK_API void oui_sk_canvas_draw_line(OuiSkCanvas canvas,
                                        float x0,
                                        float y0,
                                        float x1,
                                        float y1,
                                        OuiSkPaint paint);
OUI_SK_API void oui_sk_canvas_draw_point(OuiSkCanvas canvas, float x, float y, OuiSkPaint paint);

OUI_SK_API void oui_sk_canvas_draw_image(OuiSkCanvas canvas,
                                         OuiSkImage image,
                                         float x,
                                         float y,
                                         OuiSkPaint paint);
OUI_SK_API void oui_sk_canvas_draw_image_rect(OuiSkCanvas canvas,
                                              OuiSkImage image,
                                              const OuiSkRect* src,
                                              const OuiSkRect* dst,
                                              OuiSkPaint paint);

OUI_SK_API void oui_sk_canvas_draw_text(OuiSkCanvas canvas,
                                        const char* text,
                                        size_t len,
                                        float x,
                                        float y,
                                        OuiSkFont font,
                                        OuiSkPaint paint);
OUI_SK_API void oui_sk_canvas_draw_text_blob(OuiSkCanvas canvas,
                                             OuiSkTextBlob blob,
                                             float x,
                                             float y,
                                             OuiSkPaint paint);
...
OUI_SK_API void oui_sk_paint_set_image_filter(OuiSkPaint paint, OuiSkImageFilter filter);
OUI_SK_API void oui_sk_paint_set_mask_filter(OuiSkPaint paint, OuiSkMaskFilter filter);
OUI_SK_API void oui_sk_paint_set_color_filter(OuiSkPaint paint, OuiSkColorFilter filter);
...
OUI_SK_API void oui_sk_path_quad_to(OuiSkPath path, float cx, float cy, float x, float y);
OUI_SK_API void
oui_sk_path_cubic_to(OuiSkPath path, float c1x, float c1y, float c2x, float c2y, float x, float y);
OUI_SK_API void oui_sk_path_arc_to(OuiSkPath path,
                                   const OuiSkRect* oval,
                                   float start_angle,
                                   float sweep_angle,
                                   bool force_move_to);
...
OUI_SK_API OuiSkTypeface oui_sk_typeface_create_from_name(const char* family_name,
                                                          OuiSkFontStylePreset style);
OUI_SK_API OuiSkTypeface oui_sk_typeface_create_from_file(const char* path, int index);
OUI_SK_API OuiSkTypeface oui_sk_typeface_create_from_data(const void* data, size_t size, int index);
...
OUI_SK_API float oui_sk_font_measure_text(OuiSkFont font, const char* text, size_t len);

OUI_SK_API OuiSkTextBlob oui_sk_text_blob_create(const char* text, size_t len, OuiSkFont font);
OUI_SK_API OuiSkTextBlob oui_sk_text_shape(const char* text,
                                           size_t len,
                                           OuiSkFont font,
                                           float width,
                                           OuiSkTextAlign align,
                                           OuiSkTextDirection dir);
OUI_SK_API void oui_sk_text_blob_destroy(OuiSkTextBlob blob);

/* ─── Image ─────────────────────────────────────────────────────── */

OUI_SK_API OuiSkImage oui_sk_image_decode(const void* data, size_t size);
...
OUI_SK_API OuiSkStatus oui_sk_image_encode(OuiSkImage image,
                                           OuiSkImageFormat format,
                                           int quality,
                                           void** out_data,
                                           size_t* out_size);
...
OUI_SK_API OuiSkShader oui_sk_shader_linear_gradient(OuiSkPoint start,
                                                     OuiSkPoint end,
                                                     const OuiSkColor* colors,
                                                     const float* positions,
                                                     int count,
                                                     OuiSkTileMode tile_mode);
OUI_SK_API OuiSkShader oui_sk_shader_radial_gradient(OuiSkPoint center,
                                                     float radius,
                                                     const OuiSkColor* colors,
                                                     const float* positions,
                                                     int count,
                                                     OuiSkTileMode tile_mode);
OUI_SK_API OuiSkShader oui_sk_shader_sweep_gradient(OuiSkPoint center,
                                                    const OuiSkColor* colors,
                                                    const float* positions,
                                                    int count);
OUI_SK_API OuiSkShader oui_sk_shader_image(OuiSkImage image,
                                           OuiSkTileMode tile_x,
                                           OuiSkTileMode tile_y);
OUI_SK_API void oui_sk_shader_destroy(OuiSkShader shader);

/* ─── Image Filter ──────────────────────────────────────────────── */

OUI_SK_API OuiSkImageFilter oui_sk_image_filter_blur(float sigma_x,
                                                     float sigma_y,
                                                     OuiSkTileMode tile_mode);
OUI_SK_API OuiSkImageFilter
oui_sk_image_filter_drop_shadow(float dx, float dy, float sigma_x, float sigma_y, OuiSkColor color);
OUI_SK_API OuiSkImageFilter oui_sk_image_filter_color_filter(OuiSkColorFilter color_filter);
OUI_SK_API OuiSkImageFilter oui_sk_image_filter_compose(OuiSkImageFilter outer,
                                                        OuiSkImageFilter inner);
OUI_SK_API void oui_sk_image_filter_destroy(OuiSkImageFilter filter);

/* ─── Mask Filter ───────────────────────────────────────────────── */

OUI_SK_API OuiSkMaskFilter oui_sk_mask_filter_blur(OuiSkBlurStyle style, float sigma);
OUI_SK_API void oui_sk_mask_filter_destroy(OuiSkMaskFilter filter);

/* ─── Color Filter ──────────────────────────────────────────────── */

OUI_SK_API OuiSkColorFilter oui_sk_color_filter_blend(OuiSkColor color, OuiSkBlendMode mode);
OUI_SK_API OuiSkColorFilter oui_sk_color_filter_matrix(const float matrix[20]);
OUI_SK_API OuiSkColorFilter oui_sk_color_filter_compose(OuiSkColorFilter outer,
                                                        OuiSkColorFilter inner);
OUI_SK_API void oui_sk_color_filter_destroy(OuiSkColorFilter filter);

/* ─── Window (SDL3, temporary — replaced in SP6) ────────────────── */

OUI_SK_API OuiWindow oui_window_create(const char* title,
                                       int width,
                                       int height,
